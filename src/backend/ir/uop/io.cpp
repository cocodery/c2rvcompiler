...
void uop_mv::format_str(FILE *fp) {
    if (rs_->kind() == VREG_KIND::STK) {
        fprintf(fp, "addi %s, fp, %ld # load %s", rd_->c_str(), rs_->sinfo()->off(), rs_->c_str());
        return;
    }
...
void uop_ld::format_str(FILE *fp) {
    switch (rd_->type()) {
        case VREG_TYPE::INT:
            fprintf(fp, "lw %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
            break;
        case VREG_TYPE::PTR:
            fprintf(fp, "ld %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
            break;
        default:
            panic("unexpected");
    }
...
void uop_st::format_str(FILE *fp) {
    switch (rd_->type()) {
        case VREG_TYPE::INT:
            fprintf(fp, "sw %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
            break;
        case VREG_TYPE::PTR:
            fprintf(fp, "sd %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
            break;
        default:
            panic("unexpected");
    }
...
void uop_fld::format_str(FILE *fp) {
    // switch (rd_->type()) {
    //     case VREG_TYPE::FLT:
    fprintf(fp, "flw %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
    //         break;
    // }
}
...
void uop_fst::format_str(FILE *fp) {
    // switch (rd_->type()) {
    //     case VREG_TYPE::FLT:
    fprintf(fp, "fsw %s, %ld(%s)", rd_->c_str(), off_, rb_->c_str());
    //         break;
    // }
}
