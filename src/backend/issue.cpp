...
template <typename T>
size_t diffsetsiz(std::vector<T> &lhs, std::set<typename deduce<T>::type> &rhs) {
    std::vector<typename deduce<T>::type> recv;
    std::set_difference(lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), std::inserter(recv, recv.begin()));
    return recv.size();
}
...
static void GenAppPointInfo(Point &pt, std::array<RegAppoint, 64> &arr, std::vector<Point> &aov) {
    if (auto &&rd = pt.inst->dst; rd != 0) {
...
static void Reorder(std::vector<Point> &aov, std::list<std::shared_ptr<ASMInst>> &lst) {
    auto &&iter = lst.begin();
    ...
    while (idx != len) {
        if (ovs.empty()) panic("schedule failed");
...
static void SiFiveU740(std::shared_ptr<ASMBasicBlock> &abb) {
    ...
    for (auto &&inst : abb->asms) {
        ...
        if (rvinst->opkind == opKind::BJ) break;
