...
- [ ] 插入小程序
  ...
  - 函数 f 在 crvc.hh 中描述，可以自行选取如何实现

## 后端

> 注意，后端 ir 应该是 llvm ir 的真实翻译，所以也是 SSA。

- [x] 将常数加减乘除优化提前到后端 ir 阶段并使用链接方法优化（小小的黑魔法）
- [x] 全局符号加载后的寄存器多次复用，使用查询表和链接方法优化
- [x] 移除多余的 `load` `save`
- [x] 过大全局变量转换为全局指针变量，main 函数内额外增加 malloc 调用（XD 目前不需要，仅在全局变量上加上 onheap 标识）
- [x] 除以浮点常数转换为乘以倒数（XD 精度问题放弃）
- [x] 移除多余的跳转
- [x] 合并可合并的基本块（为后期指令调度提供更大空间，同时减少跳转次数）
- [x] 浮点数关于 2 的幂次优化（目前仅 2，注意精度问题）
- [x] 窥孔优化
