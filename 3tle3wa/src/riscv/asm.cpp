...
LA_LB::LA_LB(uint64_t rd, const char *sym) {
    // avoid format
    GENINST("la" TAB "%s" COMMA "%s", gpr[rd], sym);
}

LLA_LB::LLA_LB(uint64_t rd, const char *sym) {
    // avoid format
    GENINST("lla" TAB "%s" COMMA "%s", gpr[rd], sym);
}

LW_LB::LW_LB(uint64_t rd, const char *sym) {
    // avoid format
    GENINST("lw" TAB "%s" COMMA "%s", gpr[rd], sym);
}

LD_LB::LD_LB(uint64_t rd, const char *sym) {
    // avoid format
    GENINST("ld" TAB "%s" COMMA "%s", gpr[rd], sym);
}

SW_LB::SW_LB(uint64_t rs, const char *sym, uint64_t rt) {
    // avoid format
    GENINST("sw" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], sym, gpr[rt]);
}

SD_LB::SD_LB(uint64_t rs, const char *sym, uint64_t rt) {
    // avoid format
    GENINST("sd" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], sym, gpr[rt]);
}

FLW_LB::FLW_LB(uint64_t frd, const char *sym, uint64_t rt) {
    // avoid format
    GENINST("flw" TAB "%s" COMMA "%s" COMMA "%s", gpr[frd], sym, gpr[rt]);
}

FSW_LB::FSW_LB(uint64_t frs, const char *sym, uint64_t rt) {
    // avoid format
...
FMV_S::FMV_S(uint64_t frd, uint64_t frs) {
    // avoid format
    GENINST("fmv.s" TAB "%s" COMMA "%s", gpr[frd], gpr[frs]);
}

FABS_S::FABS_S(uint64_t frd, uint64_t frs) {
    // avoid format
    GENINST("fabs.s" TAB "%s" COMMA "%s", gpr[frd], gpr[frs]);
}

FNEG_S::FNEG_S(uint64_t frd, uint64_t frs) {
    // avoid format
...
BEQZ::BEQZ(uint64_t rs, const char *off) {
    // avoid format
    GENINST("beqz" TAB "%s" COMMA "%s", gpr[rs], off);
}

BNEZ::BNEZ(uint64_t rs, const char *off) {
    // avoid format
    GENINST("bnez" TAB "%s" COMMA "%s", gpr[rs], off);
}

BLEZ::BLEZ(uint64_t rs, const char *off) {
    // avoid format
    GENINST("blez" TAB "%s" COMMA "%s", gpr[rs], off);
}

BGEZ::BGEZ(uint64_t rs, const char *off) {
    // avoid format
    GENINST("bgez" TAB "%s" COMMA "%s", gpr[rs], off);
}

BLTZ::BLTZ(uint64_t rs, const char *off) {
    // avoid format
    GENINST("bltz" TAB "%s" COMMA "%s", gpr[rs], off);
}

BGTZ::BGTZ(uint64_t rs, const char *off) {
    // avoid format
...
BEQ::BEQ(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("beq" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BNE::BNE(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bne" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BLT::BLT(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("blt" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BGE::BGE(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bge" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BGT::BGT(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bgt" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BLE::BLE(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("ble" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BLTU::BLTU(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bltu" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BGEU::BGEU(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bgeu" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BGTU::BGTU(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
    GENINST("bgtu" TAB "%s" COMMA "%s" COMMA "%s", gpr[rs], gpr[rt], off);
}

BLEU::BLEU(uint64_t rs, uint64_t rt, const char *off) {
    // avoid format
...
LW::LW(uint64_t rd, uint64_t rs, int64_t off) {
    // avoid format
    GENINST("lw" TAB "%s" COMMA "%" PRId64 "(%s)", gpr[rd], off, gpr[rs]);
}

LD::LD(uint64_t rd, uint64_t rs, int64_t off) {
    // avoid format
    GENINST("ld" TAB "%s" COMMA "%" PRId64 "(%s)", gpr[rd], off, gpr[rs]);
}

SW::SW(uint64_t rs, uint64_t rt, int64_t off) {
    // avoid format
    GENINST("sw" TAB "%s" COMMA "%" PRId64 "(%s)", gpr[rs], off, gpr[rt]);
}

SD::SD(uint64_t rs, uint64_t rt, int64_t off) {
    // avoid format
...
ADDIW::ADDIW(uint64_t rd, uint64_t rs, int64_t imm) {
    // avoid format
    GENINST("addiw" TAB "%s" COMMA "%s" COMMA "%" PRId64, gpr[rd], gpr[rs], imm);
}

SLLIW::SLLIW(uint64_t rd, uint64_t rs, int64_t imm) {
    // avoid format
    GENINST("slliw" TAB "%s" COMMA "%s" COMMA "%" PRId64, gpr[rd], gpr[rs], imm);
}

SRLIW::SRLIW(uint64_t rd, uint64_t rs, int64_t imm) {
    // avoid format
    GENINST("srliw" TAB "%s" COMMA "%s" COMMA "%" PRId64, gpr[rd], gpr[rs], imm);
}

SRAIW::SRAIW(uint64_t rd, uint64_t rs, int64_t imm) {
    // avoid format
...
ADDW::ADDW(uint64_t rd, uint64_t rs, uint64_t rt) {
    // avoid format
    GENINST("addw" TAB "%s" COMMA "%s" COMMA "%s", gpr[rd], gpr[rs], gpr[rt]);
}

SUBW::SUBW(uint64_t rd, uint64_t rs, uint64_t rt) {
    // avoid format
    GENINST("subw" TAB "%s" COMMA "%s" COMMA "%s", gpr[rd], gpr[rs], gpr[rt]);
}

SLLW::SLLW(uint64_t rd, uint64_t rs, uint64_t rt) {
    // avoid format
    GENINST("sllw" TAB "%s" COMMA "%s" COMMA "%s", gpr[rd], gpr[rs], gpr[rt]);
}

SRLW::SRLW(uint64_t rd, uint64_t rs, uint64_t rt) {
    // avoid format
    GENINST("srlw" TAB "%s" COMMA "%s" COMMA "%s", gpr[rd], gpr[rs], gpr[rt]);
}

SRAW::SRAW(uint64_t rd, uint64_t rs, uint64_t rt) {
    // avoid format
...
FLW::FLW(uint64_t frd, uint64_t rs, int64_t off) {
    // avoid format
    GENINST("flw" TAB "%s" COMMA "%" PRId64 "(%s)", gpr[frd], off, gpr[rs]);
}

FSW::FSW(uint64_t frs, uint64_t rt, int64_t off) {
    // avoid format
...
FADD_S::FADD_S(uint64_t frd, uint64_t frs, uint64_t frt) {
    // avoid format
    GENINST("fadd.s" TAB "%s" COMMA "%s" COMMA "%s", gpr[frd], gpr[frs], gpr[frt]);
}

FSUB_S::FSUB_S(uint64_t frd, uint64_t frs, uint64_t frt) {
    // avoid format
    GENINST("fsub.s" TAB "%s" COMMA "%s" COMMA "%s", gpr[frd], gpr[frs], gpr[frt]);
}

FMUL_S::FMUL_S(uint64_t frd, uint64_t frs, uint64_t frt) {
    // avoid format
    GENINST("fmul.s" TAB "%s" COMMA "%s" COMMA "%s", gpr[frd], gpr[frs], gpr[frt]);
}

FDIV_S::FDIV_S(uint64_t frd, uint64_t frs, uint64_t frt) {
    // avoid format
